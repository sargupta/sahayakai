/**
 * Connections service — logic migrated verbatim from
 * src/app/actions/connections.ts (tranche 5, docs/API_MIGRATION_PATTERN.md).
 * Routes under /api/connections/* derive `callerId` from the
 * middleware-verified `x-user-id` header and pass it in explicitly.
 *
 * Security invariants preserved from the action module:
 *   - only the recipient (toUid) may accept a request
 *   - only a party to a request may decline/withdraw it
 *   - only a participant may disconnect a connection
 */
...
export async function sendConnectionRequest(
    ...
    try {
        ...
        logger.error('sendConnectionRequest failed', err, 'CONNECTIONS', { userId: fromUid });
        throw err;
    }
}
...
export async function acceptConnectionRequest(callerId: string, requestId: string): Promise<void> {
    try {
        const db = await getDb();
        ...
        logger.error('acceptConnectionRequest failed', err, 'CONNECTIONS', { userId: callerId });
        throw err;
    }
}
...
export async function declineConnectionRequest(callerId: string, requestId: string): Promise<void> {
    try {
        const db = await getDb();
        ...
        logger.error('declineConnectionRequest failed', err, 'CONNECTIONS', { userId: callerId });
        throw err;
    }
}
...
export async function disconnect(callerId: string, otherUid: string): Promise<void> {
    try {
        const db = await getDb();
        ...
        logger.error('disconnect failed', err, 'CONNECTIONS', { userId: callerId });
        throw err;
    }
}
...
export async function getMyConnectionData(callerId: string): Promise<MyConnectionData> {
    try {
    const db = await getDb();
    ...
    return dbAdapter.serialize({ connectedUids, sentRequestUids, receivedRequests }) as MyConnectionData;
        ...
        logger.error('getMyConnectionData failed', err, 'CONNECTIONS', { userId: callerId });
        throw err;
    }
}
