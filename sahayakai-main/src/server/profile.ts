/**
 * Profile service — migrated from src/app/actions/profile.ts (tranche 5,
 * docs/API_MIGRATION_PATTERN.md). Logic is verbatim; the only changes are:
 *  - `"use server"` directive dropped (plain server module),
 *  - `revalidatePath("/my-profile")` calls dropped (client pages fetch via
 *    the API boundary and manage their own state; noted in the PR),
 *  - `addCertificationAction` takes a plain object instead of FormData
 *    (the JSON API boundary has no FormData; the client wrapper preserves
 *    the original FormData signature).
 */
 ...
 * convention used in the connections domain.
 */
 ...
 * via dedicated sanitised readers. This one returns the FULL profile and
 ...
 *
 * Wave 1: now derives uid from session — any userId supplied by the client
 * is ignored. Previously any caller could add certifications to any user's
 * profile by passing a different uid.
 */
export async function addCertificationAction(input: CertificationInput) {
    const userId = await requireAuth();
    try {
        const certName = input.certName as string;
        const issuingBody = input.issuingBody as string;
        const issueDate = input.issueDate as string;
