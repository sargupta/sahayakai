/**
 * App-feedback service — migrated verbatim from src/app/actions/feedback.ts
 * (tranche 5, docs/API_MIGRATION_PATTERN.md). Served by POST /api/feedback/app
 * (the plain /api/feedback route is the pre-existing per-content feedback
 ...
 * (NOTE: since the 2026-06-01 middleware gate, unauthenticated POSTs are
 * already rejected at the edge in production — the optional-auth path here is
 * preserved for parity and for dev mode.)
 */
