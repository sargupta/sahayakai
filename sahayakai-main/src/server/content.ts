/**
 * Content-library domain service — tranche 5 API-boundary migration.
 *
 * Logic moved verbatim from src/app/actions/content.ts (deleted). Functions
 * take the middleware-verified `userId` as their first argument; the
 * /api/content/{library,search,pdf-download,storage-test} routes are thin
 * shells (auth → validate → call → JSON).
 *
 * Wave 1/2b security posture preserved:
 * - uid always comes from the verified header, never the client body.
 * - Error messages returned to clients are scrubbed (no bucket names,
 *   Secret-Manager paths, or gRPC codes).
 * - storage-test stays admin-gated via validateAdmin.
 */
...
/**
 * Get the caller's own content (uid from the verified header only).
 */
 ...
 * Searches user content and boosts matches based on teacher profile
 * (Grade, Subject, Language).
 */
export async function searchContent(userId: string, query: string): Promise<BaseContent[]> {
    try {
...
export async function saveToLibrary(userId: string, type: ContentType, title: string, data: any): Promise<{ success: boolean; id?: string; error?: string }> {
    try {
        ...
        // Migration note: the server action called revalidatePath("/my-library")
        // + revalidatePath("/impact-dashboard") here. Both pages are client
        // components that fetch on mount, so the API boundary relies on the
        // client-side refresh instead (see docs/API_MIGRATION_PATTERN.md §7).
 ...
 * reachable. Admin-gated (Wave 1) — throws ForbiddenError for non-admins.
 */
export async function testStorageConnection(userId: string): Promise<{ success: boolean; message: string }> {
