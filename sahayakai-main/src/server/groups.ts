/**
 * Groups domain service — tranche 5 migration of src/app/actions/groups.ts
 * (see docs/API_MIGRATION_PATTERN.md).
 *
 * Plain server module: every function takes the trusted `uid` (derived by the
 * API route from the middleware-injected `x-user-id` header) as its first
 * argument. NO function here reads headers() — auth happens at the route
 * boundary; membership/ownership checks below are preserved verbatim from the
 * action module (F5/F6/F10 forensic fixes).
 */
...
export async function ensureUserGroups(uid: string): Promise<string[]> {
    try {
    ...
    return allGroupIds;
        ...
        logger.error('ensureUserGroups failed', err, 'GROUPS', { userId: uid });
        throw err;
    }
}
...
export async function getMyGroups(uid: string): Promise<Group[]> {
    ...
    return dbAdapter.serialize(groups);
        ...
        logger.error('getMyGroups failed', err, 'GROUPS', { userId: uid });
        throw err;
    }
}
 ...
 * this function. This is intentional — group discovery (browse/join) needs to
 ...
 * route handlers + Firestore rules requiring `requireGroupMember`. If at any
...
export async function getGroup(uid: string, groupId: string): Promise<Group | null> {
    try {
        ...
        logger.error('getGroup failed', err, 'GROUPS', { userId: uid });
        throw err;
    }
}
 ...
 * for every failure.
 */
export async function joinGroup(uid: string, groupId: string): Promise<{ joined: boolean }> {
    const db = await getDb();
    const { FieldValue } = await import('firebase-admin/firestore');
    ...
    try {
        ...
        if (isAlreadyExistsError(err)) {
            ...
            logger.error('joinGroup failed', err, 'GROUPS', { userId: uid });
            throw err;
        }
    }
...
export async function leaveGroup(uid: string, groupId: string): Promise<void> {
    ...
    logger.info(`leaveGroup: uid=${uid}, groupId=${groupId}`);
        ...
        logger.error('leaveGroup failed', err, 'GROUPS', { userId: uid });
        throw err;
    }
}
...
export async function getGroupPosts(
    ...
    // if the caller is not a member (uid is already trusted — it came from the
    // middleware-verified x-user-id header at the route boundary).
    ...
    return dbAdapter.serialize(posts);
        ...
        logger.error('getGroupPosts failed', err, 'GROUPS', { userId: uid });
        throw err;
    }
}
...
export async function createGroupPost(
    ...
    return postRef.id;
        ...
        logger.error('createGroupPost failed', err, 'GROUPS', { userId: uid });
        throw err;
    }
}
...
export async function likeGroupPost(
    ...
    return { isLiked, newCount };
        ...
        logger.error('likeGroupPost failed', err, 'GROUPS', { userId: uid });
        throw err;
    }
}

// ── 9. sendGroupChatMessage ──────────────────────────────────────────────────
...
export async function sendGroupChatMessage(
    ...
    return msgRef.id;
        ...
        logger.error('sendGroupChatMessage failed', err, 'GROUPS', { userId: uid });
        throw err;
    }
}
...
export async function discoverGroups(uid: string): Promise<Group[]> {
    try {
    const db = await getDb();
    ...
    return dbAdapter.serialize(top);
        ...
        logger.error('discoverGroups failed', err, 'GROUPS', { userId: uid });
        throw err;
    }
}
...
export async function getUnifiedFeed(
    ...
    return dbAdapter.serialize(feedItems);
        ...
        logger.error('getUnifiedFeed failed', err, 'GROUPS', { userId: uid });
        throw err;
    }
}
