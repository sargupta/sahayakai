                ...
                if (TERMINAL_FAIL.has(status)) {
                    ...
                    setMessage(
                        status === 'no-answer' || status === 'busy'
                            ? 'We could not reach your phone. Please check the number and try again tomorrow.'
    ...
    if (phase === 'done') {
        return (
            <section className="space-y-4 rounded-xl border bg-card p-6 text-center shadow-sm">
                ...
                <h2 className="text-xl font-semibold">What just happened</h2>
                <p className="text-sm text-muted-foreground">
                    That voice you heard was SahayakAI making a parent call. The same call can go to
                    every parent in your register: absence alerts, meeting invitations, exam
                    reminders, even a praise call for a child who did something good this week.
                </p>
                <p className="text-sm text-muted-foreground">
                    You choose the message once. SahayakAI makes every call, in each parent&apos;s own
                    language, and tells you who picked up.
                </p>
                <Button asChild size="lg" className="w-full sm:w-auto">
                    <Link href="/signup?ref=try-call">Send your first 10 parent calls free</Link>
                </Button>
                <p className="text-xs text-muted-foreground">
                    Works with your existing class register. Setup takes less time than one parent
                    call used to.
                </p>
            </section>
        );
        ...
        <section className="rounded-xl border bg-card p-6 shadow-sm">
            ...
            <form onSubmit={onSubmit} className="space-y-4" aria-busy={phase === 'submitting'}>
                ...
                <label className="flex items-start gap-2 text-xs text-muted-foreground">
                    ...
                    />
                    <span>
                        I am requesting a one-time demo call to my own number. SahayakAI will call
                        this number once and send one WhatsApp follow-up. No spam, ever.
                    </span>
                </label>
                ...
                {phase === 'calling' ? (
                    <div className="flex items-center justify-center gap-2 rounded-md bg-muted p-4 text-sm font-medium">
                        ...
                        Your phone will ring in a few seconds. Pick up and listen.
                    </div>
                ) : (
                    <Button
                        type="submit"
                ...
                <p className="text-center text-xs text-muted-foreground">
                    Free. No signup needed to hear the call.
                </p>
            </form>
        </section>
    );
