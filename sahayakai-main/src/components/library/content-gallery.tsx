...
import { BaseContent, ContentType, LANGUAGE_TO_ISO } from "@/types";
...
// (attacker-influenced), so unescaped interpolation was a stored-XSS sink:
// a saved resource containing `<script>` executed on download/print.
...
// Component-local i18n for the exported/printed artifact footer. The footer
// lives inside generated HTML (no JSX/t() at that node), so we resolve by the
// app UI language (uiLangCode) using this table + a BCP-47 locale map.
const GENERATED_BY: Record<string, string> = {
    en: "Generated by SahayakAI",
    hi: "SahayakAI द्वारा निर्मित",
    mr: "SahayakAI द्वारे तयार केले",
    bn: "SahayakAI দ্বারা তৈরি",
    pa: "SahayakAI ਦੁਆਰਾ ਤਿਆਰ ਕੀਤਾ ਗਿਆ",
    gu: "SahayakAI દ્વારા તૈયાર કરાયેલ",
    or: "SahayakAI ଦ୍ୱାରା ପ୍ରସ୍ତୁତ",
    ta: "SahayakAI மூலம் உருவாக்கப்பட்டது",
    te: "SahayakAI ద్వారా రూపొందించబడింది",
    kn: "SahayakAI ಮೂಲಕ ರಚಿಸಲಾಗಿದೆ",
    ml: "SahayakAI വഴി സൃഷ്ടിച്ചത്",
};

const UI_LANG_TO_BCP47: Record<string, string> = {
...
<head>
    <meta charset="UTF-8">
    <title>${escapeHtml(content.title || content.topic || 'Content')}</title>
    <style>
...
<body>
    <h1>${escapeHtml(content.title || content.topic || 'Content')}</h1>
    <div class="metadata">
        <p><strong>${t("Type:")}</strong> ${escapeHtml(type)}</p>
        ${content.grade_level ? `<p><strong>${t("Class:")}</strong> ${escapeHtml(content.grade_level)}</p>` : ''}
        ${content.language ? `<p><strong>${t("Language:")}</strong> ${escapeHtml(content.language)}</p>` : ''}
        ${content.subject ? `<p><strong>${t("Subject:")}</strong> ${escapeHtml(content.subject)}</p>` : ''}
    </div>
    <div class="content">
    ...
    <footer style="margin-top: 40px; padding-top: 20px; border-top: 1px solid #ccc; color: #7f8c8d; font-size: 0.9em;">
        <p>${GENERATED_BY[uiLangCode] ?? GENERATED_BY.en} | ${new Date().toLocaleDateString(UI_LANG_TO_BCP47[uiLangCode] ?? "en-IN")}</p>
    </footer>
</body>
</html>`;
    ...
    const formatLessonPlan = (content: any): string => {
        ...
        if (content.sections) {
            content.sections.forEach((section: any) => {
                html += `<h2>${escapeHtml(section.heading || section.title)}</h2>`;
                // Escape body and preserve line breaks — content is untrusted.
                html += `<div style="white-space: pre-wrap;">${escapeHtml(section.content || section.body || '')}</div>`;
            });
        }
    ...
    const formatQuiz = (content: any): string => {
        ...
        if (content.questions && Array.isArray(content.questions)) {
            content.questions.forEach((q: any, i: number) => {
                ...
                if (q.answer) {
                    html += `<p><strong>${t("Answer:")}</strong> ${escapeHtml(q.answer)}</p>`;
                }
            });
        }
        html += '</div>';
    ...
    const formatRubric = (content: any): string => {
        ...
        if (content.criteria && Array.isArray(content.criteria)) {
            ...
            content.criteria.forEach((criterion: any) => {
                html += `<tr><td>${escapeHtml(criterion.name || criterion.title)}</td><td>${escapeHtml(criterion.description || '')}</td></tr>`;
            });
            html += '</tbody></table>';
        }
        html += '</div>';
