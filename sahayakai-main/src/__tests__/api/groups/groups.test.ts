/**
 * Contract tests for src/server/groups.ts (tranche 5: migrated from
 * src/app/actions/groups.ts — the uid is now an explicit first argument
 * supplied by the /api/groups/** routes from the trusted x-user-id header).
 * Focuses on ensureUserGroups: short-circuit, privacy fix, auto-join behavior.
 * The old per-action Unauthorized assertions are re-expressed as route-level
 * 401 checks below (full route coverage: groups-routes-auth.test.ts).
 */

// ── Mock: next/headers (auth-helpers imports it at module scope) ─────────────
...
describe('ensureUserGroups', () => {
    ...
    describe('short-circuit when groupsInitialized === true', () => {
        it('returns existing groupIds immediately without creating any groups', async () => {
            ...
            const result = await ensureUserGroups('test-uid');
    ...
    describe('school group privacy fix — no auto-join', () => {
        it('creates school group doc but does NOT create a member doc for the user', async () => {
            ...
            await ensureUserGroups('test-uid');
    ...
    describe('auth', () => {
        it('route returns 401 when x-user-id header is missing', async () => {
            const { POST } = await import('@/app/api/groups/ensure/route');
            const res = await POST({
                headers: { get: () => null },
                nextUrl: new URL('http://localhost/api/groups/ensure'),
            } as any);
        ...
        it('throws when user profile not found', async () => {
            ...
            await expect(ensureUserGroups('test-uid')).rejects.toThrow('User profile not found');
        });
    });
});

// ── Phase 1 security regression: getGroupPosts membership check ───────

import { getGroupPosts } from '@/server/groups';

describe('getGroupPosts — Phase 1 membership gate', () => {
    beforeEach(() => {
        Object.keys(store).forEach(k => delete store[k]);
        createCalls.length = 0;
        mockHeadersMap.set('x-user-id', 'test-uid');
    ...
    it('throws Forbidden when caller is not a member of the group', async () => {
        ...
        await expect(getGroupPosts('test-uid', 'private-group-id'))
    ...
    it('route returns 401 when x-user-id header is missing', async () => {
        ...
        const res = await GET(
            {
                headers: { get: () => null },
                nextUrl: new URL('http://localhost/api/groups/any-group/posts'),
            } as any,
            { params: Promise.resolve({ groupId: 'any-group' }) } as any,
        );
        expect(res.status).toBe(401);
    });
    ...
    it('returns posts when caller IS a member', async () => {
        ...
        const posts = await getGroupPosts('test-uid', 'test-group');
        expect(Array.isArray(posts)).toBe(true);
    });
});

// ── Phase 2: joinGroup surfaces real errors instead of silently succeeding

import { joinGroup } from '@/server/groups';

describe('joinGroup — Phase 2 error handling', () => {
    beforeEach(() => {
        Object.keys(store).forEach(k => delete store[k]);
        createCalls.length = 0;
        mockHeadersMap.set('x-user-id', 'test-uid');
    ...
    it('returns { joined: true } when the caller becomes a new member', async () => {
        const result = await joinGroup('test-uid', 'existing-grp');
    ...
    it('throws "Group not found" when the group does not exist', async () => {
        await expect(joinGroup('test-uid', 'nonexistent-grp'))
