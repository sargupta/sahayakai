/**
 * Notification fanout tests for createGroupPost and likeGroupPost.
...
describe('group notification fanout', () => {
    ...
    describe('createGroupPost → NEW_GROUP_POST fanout', () => {
        it('writes one notification per member, skipping the author, with localized message per recipient', async () => {
            ...
            await createGroupPost('author-uid', groupId, 'Hello class', 'share');
    ...
    describe('likeGroupPost → GROUP_POST_LIKE notification', () => {
        it('notifies the post author when a different user likes', async () => {
            ...
            await likeGroupPost('liker-uid', groupId, postId);
            await new Promise((r) => setTimeout(r, 0));
