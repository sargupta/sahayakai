...
// getPublicProfileAction migrated to src/server/profile.ts (tranche 5,
// served by GET /api/profile/public/[uid]); same F10-02 logic.
import { getPublicProfileAction } from '@/server/profile';
// Community actions migrated to src/server/community.ts (tranche 5, served
// by /api/community/**); same F10-03/04/05 guards — the caller uid is now an
// explicit first argument (the route derives it from the trusted x-user-id
// header) instead of a headers() read inside the function.
import {
    sendChatMessage,
    followTeacher,
    toggleLike,
...
describe('F10-03 sendChatMessage audioUrl validation', () => {
    beforeEach(() => {
    ...
    it('rejects tracking-pixel http URL', async () => {
        await expect(
            sendChatMessage('author-uid', 'hi', 'http://evil.com/track.mp3?u=victim'),
        ).rejects.toThrow(/Invalid audio URL/);
    });

    it('rejects non-firebasestorage https host', async () => {
        await expect(
            sendChatMessage('author-uid', 'hi', 'https://evil.com/track.mp3'),
        ).rejects.toThrow(/Invalid audio URL/);
    });

    it('rejects audioUrl longer than 1024 chars', async () => {
        const long = 'https://firebasestorage.googleapis.com/' + 'a'.repeat(1100);
        await expect(sendChatMessage('author-uid', 'hi', long)).rejects.toThrow(/Invalid audio URL/);
    });
...
describe('F10-04 followTeacher self-follow', () => {
    it('rejects when followingId === callerUid', async () => {
        await expect(followTeacher('me-uid', 'me-uid')).rejects.toThrow(/Cannot follow yourself/);
    });

    it('allows following another teacher', async () => {
        docStore.set('users/me-uid', { displayName: 'Me' });
        await expect(followTeacher('me-uid', 'other-uid')).resolves.toBeUndefined();
    });
});
...
describe('F10-05 toggleLike self-like (posts)', () => {
    it('rejects when post.authorId === callerUid', async () => {
        docStore.set('posts/my-post', { authorId: 'me-uid', text: 'hi' });
        await expect(toggleLike('me-uid', 'my-post')).rejects.toThrow(/Cannot like your own post/);
    });

    it('allows liking another teacher’s post', async () => {
        docStore.set('posts/other-post', { authorId: 'other-uid', text: 'hi' });
        await expect(toggleLike('me-uid', 'other-post')).resolves.toBeUndefined();
    });
});

describe('F10-05 likeResource self-like (resources)', () => {
    it('rejects when resource.authorId === callerUid', async () => {
        docStore.set('library_resources/my-res', { authorId: 'me-uid', title: 'mine' });
        await expect(likeResource('me-uid', 'my-res')).rejects.toThrow(/Cannot like your own resource/);
    });
});
