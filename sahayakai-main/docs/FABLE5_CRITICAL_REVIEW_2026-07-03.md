...
13. **The CI that everyone believes in does not exist.** *(Corrected + upgraded during execution, 2026-07-03 — reality was worse than first written.)* Two compounding failures:
    - **The entire `sahayakai-main/.github/workflows/` directory is inert.** GitHub only reads workflows from the repo root. `quality-gates.yml` (all 8 gates: no-verify scan, schema drift, console.log, typecheck…), `cloud-run.yml`, and `genkit-eval.yml` had **never executed once** — including the weekly eval cron. The only live workflows were the root's `test.yml` plus two deliberately dispatch-only deploy workflows.
    - **The live `test.yml` swallowed every test failure.** Its jest step ended in `… | tee test-output.txt`; GitHub Actions' default shell has no `pipefail`, so the step's exit code was `tee`'s — always 0. Measured today: **50 failing tests across 16 suites sat green on `develop`**, merged unnoticed over weeks.
    - Fixed in tranche 1: pipe removed, `quality-gates.yml` + `genkit-eval.yml` moved to root (paths adjusted), the inert push-triggered `cloud-run.yml` deleted (moving it live would have raced `safe-deploy.sh`), and all 16 failing suites repaired. The general lesson: **a gate you have never watched fail is a gate that isn't running.**
