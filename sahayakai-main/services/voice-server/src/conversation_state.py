...
PHASE_GUIDANCE: dict[ConversationPhase, str] = {
    ConversationPhase.OPENING: (
        ...
        "Your call reason is: {reason}. "
        "Do NOT dump the full problem yet — ease in. "
        "Ask ONE gentle question about {student_name} at home. "
        "For exam issues, say 'padhai mein thodi dikkat' NOT the score. "
        "For behavior, say 'thoda concerned hoon' NOT the complaint. "
        "For fees, start with academic concern first, fees comes later. "
        "If parent already started talking about something, respond to THAT. "
        "Do NOT ask 'how can I help' — you called them.]"
    ),
    ConversationPhase.EXPLORE: (
        "{prefix}: You're understanding the situation. "
        "Listen to what parent said and acknowledge warmly. "
        "If they shared a problem (health, money, family), show empathy first. "
        "If they're defensive about behavior, DON'T argue — say 'bilkul, milke dekhte hain'. "
        "If they seem embarrassed about marks/fees, reassure — 'koi baat nahi'. "
        "Ask ONE follow-up if needed, or move to suggestion if you understand enough. "
        "If parent brings up something unrelated, address THAT — don't force your agenda.]"
    ),
    ConversationPhase.ADVISE: (
        "{prefix}: You understand the situation now. "
        "Give ONE practical suggestion if appropriate. "
        "For academic issues: offer extra help, suggest home study routine. "
        "For behavior: suggest meeting at school, working together. "
        "For fees: say you'll talk to principal, reassure. "
        "Or if parent raised a concern, address it. "
        "Then check: 'aur koi baat hai?' "
        "If parent says no or seems ready to end, wrap up next turn.]"
    ),
    ConversationPhase.CLOSE: (
        "{prefix}: Wrap up the call warmly. "
        "Thank the parent for their time. Mention {student_name} positively. "
        "Say goodbye naturally — 'achha ji, dhyan rakhiyega'. "
        "If parent raises a NEW concern, briefly address it, then still close.]"
    ),
}
