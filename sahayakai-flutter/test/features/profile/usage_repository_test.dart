...
void main() {
  ...
  group('UsageSummary.fromJson', () {
    ...
    test('reuses the plan-badge mapping (legacy institution -> premium)', () {
      ...
      expect(
        UsageSummary.fromJson(usageJson(plan: 'gold')).plan,
        PlanBadge.gold,
      );
    });
    ...
    test('groups metered features first, each group in server order', () {
      ...
      expect(summary.features.map((f) => f.feature).toList(), <String>[
        'lesson-plan',
        'quiz',
        'visual-aid',
        'instant-answer',
    ...
    test('reads used and limit, and computes the meter fraction', () {
      final summary = UsageSummary.fromJson(usageJson());
      final lessonPlan = summary.features.firstWhere(
        (f) => f.feature == 'lesson-plan',
      );
    ...
    test('a full feature clamps the fraction to 1, never past the track', () {
      final quiz = UsageSummary.fromJson(
        usageJson(),
      ).features.firstWhere((f) => f.feature == 'quiz');
      // 5 of 5 used.
      expect(quiz.fraction, 1.0);

      // And an over-quota count (used > limit) still clamps.
      const over = FeatureUsage(feature: 'x', used: 12, limit: 10);
      expect(over.fraction, 1.0);
    });

    test('a -1 limit is unlimited: no fraction, no bar', () {
      final instant = UsageSummary.fromJson(
        usageJson(),
      ).features.firstWhere((f) => f.feature == 'instant-answer');
    ...
    test(
      'is tolerant: numeric strings parse, a missing limit is unlimited',
      () {
        ...
        final mystery = summary.features.firstWhere(
          (f) => f.feature == 'mystery',
        );
    ...
    test('an absent or non-map usage yields no features, not a crash', () {
      expect(
        UsageSummary.fromJson(<String, dynamic>{'plan': 'free'}).features,
        isEmpty,
      );
      expect(
        UsageSummary.fromJson(<String, dynamic>{
          'plan': 'free',
          'usage': 'nope',
        }).features,
        isEmpty,
  ...
  group('UsageRepository.fetchUsage', () {
    ...
    test(
      'a 401 surfaces as the typed unauthorized exception (the signed-out cue)',
      () {
  ...
  group('usageSummaryProvider', () {
    test('resolves to the decoded summary', () async {
      final container = containerWith([
        apiClientProvider.overrideWithValue(
          FakeApiClient(getResponse: usageJson()),
        ),
      ]);
    ...
    test('propagates the 401 so the section can degrade', () {
      final container = containerWith([
        apiClientProvider.overrideWithValue(
          FakeApiClient(error: _kUnauthorized),
        ),
      ]);
