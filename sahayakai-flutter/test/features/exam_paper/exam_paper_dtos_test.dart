...
void main() {
  group('ExamPaperRequestDto', () {
    ...
    test('never sends server-injected fields', () {
      ...
      for (final field in ['userId', 'teacherContext', 'duration', 'maxMarks']) {
        expect(
    ...
    test('always sends chapters, even empty (a valid "all chapters" signal)',
        () {
      final json = ExamPaperRequestDto.fromDomain(
        const ExamPaperRequest(
          board: 'CBSE',
  ...
  group('examPaperNeedsChapters (the blueprint rule)', () {
    test('a blueprinted CBSE Class 9/10 Maths/Science combo does NOT need them',
        () {
      expect(examPaperNeedsChapters('CBSE', 'Class 10', 'Mathematics'), isFalse);
      expect(examPaperNeedsChapters('CBSE', 'Class 10', 'Science'), isFalse);
      expect(examPaperNeedsChapters('CBSE', 'Class 9', 'Mathematics'), isFalse);
    ...
    test('everything else DOES need at least one chapter', () {
      ...
      expect(examPaperNeedsChapters('ICSE / ISC', 'Class 10', 'Science'), isTrue);
      expect(examPaperNeedsChapters('CBSE', 'Nursery', 'Mathematics'), isTrue);
    });

    test('a still-unset required field never double-reports (returns false)',
        () {
      expect(examPaperNeedsChapters(null, 'Class 10', 'Mathematics'), isFalse);
  ...
  group('ExamPaperResponseDto.resultFrom', () {
    test('a 200 body decodes to a READY paper (sections, keys, blueprint, pyq)',
        () {
      ...
      expect(paper.title, startsWith('CBSE Class 10 Mathematics Sample Paper'));
    ...
    test('keeps the raw JSON verbatim, so a save round-trips byte-for-byte', () {
      final json = examPaperJson();
      final ready = ExamPaperResponseDto.resultFrom(json) as ExamPaperReady;
      expect(ready.raw, same(json));
    });

    test('a 202 body decodes to the distinct IN-PROGRESS state, not a paper',
        () {
      final result = ExamPaperResponseDto.resultFrom(examPaperInProgressJson());
        ...
        'sections': <Map<String, dynamic>>[
          <String, dynamic>{
            ...
            'questions': <Map<String, dynamic>>[
              <String, dynamic>{'number': 1, 'text': 'A question with no marks.'},
            ],
          },
        ],
      });
