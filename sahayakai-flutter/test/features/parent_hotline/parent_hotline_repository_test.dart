...
void main() {
  ...
  group('createOutreach — POST /api/attendance/outreach', () {
    test('happy path returns the outreachId and posts the right body', () async {
      final client = FakeApiClient(postResponse: {'outreachId': 'o-123'});
      final id = await ParentHotlineRepository(client).createOutreach(request());
    ...
    test('a non-premium 403 (ownership Forbidden) is NOT wrapped as premium',
        () async {
      final client = FakeApiClient(
        postError: const ApiException(
          ApiErrorKind.forbidden,
    ...
    test('429 → OutreachDedupException with retryAfterSeconds parsed', () async {
      ...
      await expectLater(
        () => ParentHotlineRepository(client).createOutreach(request()),
        throwsA(isA<OutreachDedupException>()
            .having((e) => e.retryAfterSeconds, 'retryAfterSeconds', 240)),
      );
    });

    test('401 → stays an auth ApiException (not a ParentHotlineException)',
        () async {
  ...
  group('placeCall — POST /api/attendance/call', () {
    test('happy path returns the callSid and posts ONLY {outreachId, '
      ...
      final sid = await ParentHotlineRepository(client)
      ...
      expect(client.posts.single.data,
          {'outreachId': 'o-123', 'parentLanguage': 'Kannada'});
      expect((client.posts.single.data as Map).containsKey('parentPhone'),
          isFalse); // F9-001
    });

    test('422 unsupported-language → UnsupportedCallLanguageException(language)',
        () async {
      final client = FakeApiClient(
        postError: const ApiException(
          ApiErrorKind.badResponse,
      ...
      await expectLater(
        () => ParentHotlineRepository(client)
            .placeCall(outreachId: 'o-1', parentLanguage: 'Odia'),
        throwsA(isA<UnsupportedCallLanguageException>()
            .having((e) => e.language, 'language', 'Odia')),
      );
    });

    test('422 no-valid-phone → NoParentPhoneException (disambiguated by msg)',
        () async {
      final client = FakeApiClient(
        postError: const ApiException(
          ApiErrorKind.badResponse,
  ...
  group('pollSummary — GET /api/attendance/call-summary', () {
    test('decodes the call result and queries by outreachId', () async {
      ...
      final result =
          await ParentHotlineRepository(client).pollSummary('o-123');
