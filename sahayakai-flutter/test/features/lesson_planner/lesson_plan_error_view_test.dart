...
import 'package:sahayakai/features/lesson_planner/presentation/widgets/lesson_plan_error_view.dart';
...
/// upgrade, wait or sign in — not one generic error. The release-gate sweep
/// caught the plan-limit states (403 / 429) dead-ending with no way to act;
/// these tests pin that they now offer an actionable "See pricing" button that
...
void main() {
  ...
  testWidgets('400 -> rephrase hint with a retry', (tester) async {
    ...
    await tester.pumpAndSettle();
    expect(find.textContaining('rephrase the topic'), findsOneWidget);
    expect(find.text('Try again'), findsOneWidget);
  });
  ...
  testWidgets('a non-ApiException falls back to the generic error',
    ...
    await tester.pumpWidget(
      host(
        LessonPlanErrorView(error: StateError('boom'), onRetry: () {}),
        linkOpener: _FakeLinkOpener(),
      ),
    );
    await tester.pumpAndSettle();
