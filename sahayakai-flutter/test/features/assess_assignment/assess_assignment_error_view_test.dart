...
/// Every failed assessment maps to the *right* recovery UI — retry, re-upload,
/// upgrade, wait or sign in — not one generic error. The headline case is the
/// DAY-BUDGET limit: it is distinct from the monthly usage limit and must NOT
/// offer a retry (a clock-reset quota would only fail again).
void main() {
  Future<void> pump(
    WidgetTester tester,
    Object error, {
  ...
  group('the day-budget limit is a distinct, no-retry state', () {
    ...
    testWidgets('429 USAGE_LIMIT_REACHED is the DIFFERENT monthly state', (
      ...
      expect(
        find.text('You have reached your assessment limit'),
        findsOneWidget,
      );
    ...
    testWidgets('the daily state reads its code from the raw body', (
      ...
      await pump(
        ...
        ApiException(
          ...
          "You've used all your assessments for today.",
          statusCode: 429,
          raw: _dio(429, const {
  ...
  group('the other states', () {
    ...
    testWidgets('401 -> sign-in message, no retry', (tester) async {
      ...
      expect(
        find.text('Please sign in again to grade an assignment.'),
        findsOneWidget,
      );
      expect(find.text('Try again'), findsNothing);
    });
