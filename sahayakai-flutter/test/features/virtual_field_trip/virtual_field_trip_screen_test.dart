...
void main() {
  ...
  group('states', () {
    ...
    testWidgets(
      'data renders the itinerary document with numbered stop cards',
      (tester) async {
        await tester.pumpWidget(
          _host(
            overrides: [
    ...
    testWidgets(
      'the still-generating 202 renders the calm panel, not an error',
      (tester) async {
        await tester.pumpWidget(
          _host(
            overrides: [
      ...
      await tester.pumpWidget(
        _host(
          prefill: const ToolPrefill(
            ...
            gradeLevel: 'Class 10',
            subject:
                'Science', // this screen has no subject field; must be ignored, not crash
            language: 'kn',
          ),
        ),
      );
