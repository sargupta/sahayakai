...
void main() {
  ...
  group('VisualAidRepository.generate', () {
    test('a 200 decodes the drawing and posts the DTO body', () async {
      ...
      final aid =
          await container.read(visualAidRepositoryProvider).generate(request);
    ...
    test('a 200 with no image decodes to the empty state, not an error',
        () async {
      final client = FakeApiClient(postResponse: visualAidJson(withImage: false));
      final container = containerWith(client);
