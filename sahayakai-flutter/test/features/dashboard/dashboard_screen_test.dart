...
void main() {
  ...
  group('greeting', () {
    ...
    testWidgets('uses the teacher\'s name once the profile loads', (
      ...
      await pumpDashboard(
        tester,
        docs: FakeProfileDocSource(doc: _teacherDoc()),
      );
  ...
  group('tool tiles', () {
    ...
    testWidgets(
      'the instant answer tile opens the real instant answer screen',
      (tester) async {
        await pumpDashboard(tester);
  ...
  group('recent work states', () {
    ...
    testWidgets(
      'any other failure shows the error state, with no raw exception',
      (tester) async {
    ...
    testWidgets('retry re-runs the fetch and recovers', (tester) async {
      ...
      expect(
        contentReads(),
        hasLength(2),
        reason: 'retry must re-run the fetch',
      );
      expect(find.byType(OfflineView), findsNothing);
      expect(find.text('Photosynthesis for Class 6'), findsOneWidget);
    });
  ...
  group('setup nudge (a nudge, never a gate)', () {
    testWidgets('appears only when the profile really is empty', (
      tester,
    ) async {
