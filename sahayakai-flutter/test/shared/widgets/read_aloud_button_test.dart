...
void main() {
  ...
  testWidgets('tapping Listen synthesises via /api/tts and plays the result', (
    ...
    await tester.pumpWidget(
      _host(const ReadAloudButton(text: 'Read me aloud.', language: 'Hindi'), [
    ...
    expect(
      ttsPosts,
      hasLength(1),
      reason: 'the TTS repository was driven once',
    );
  ...
  testWidgets('a second tap stops playback and resets to Listen', (
    ...
    final player = FakeAudioPlayerService();
    await tester.pumpWidget(
      _host(const ReadAloudButton(text: 'x', language: 'en'), [
        apiClientProvider.overrideWithValue(client),
        audioPlayerServiceProvider.overrideWithValue(player),
      ]),
    );
  ...
  testWidgets(
    ...
    (tester) async {
      ...
      await tester.pumpWidget(
        _host(
          const ReadAloudButton(
            text: 'a long deliverable to narrate',
            language: 'en',
          ),
          overrides,
        ),
      ...
      await tester.tap(find.text('Listen'));
      await tester
          .pump(); // _toggle runs up to `await speak()`; synth is in flight
      ...
      expect(
        ...
        reason:
            'a clip started during synthesis must be stopped when the view '
            'disposes mid-synthesis, so it does not outlive the screen',
      );
    },
  );

  testWidgets('natural completion resets the control to Listen', (
    tester,
  ) async {
    final client = FakeApiClient(postResponse: {'audioContent': _b64});
    final player = FakeAudioPlayerService();
