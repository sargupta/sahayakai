...
/// decides whether the teacher should retry, rephrase, upgrade, wait, or stop.
///
...
///                                   the image budget resets on a clock)
...
///     / 504 IMAGE_GENERATION_TIMEOUT  (the route's slow-model timeout is a 504,
///                                   which maps to the retryable server branch)
///   - 422 IMAGE_GENERATION_EMPTY -> "fewer labels" rephrase hint + retry
///   - 400 badResponse            -> safety / rephrase hint + retry
///   - timeout / other 4xx        -> retryable message + retry
class VisualAidErrorView extends ConsumerWidget {
  const VisualAidErrorView({super.key, required this.error, this.onRetry});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    switch (api.kind) {
      ...
      case ApiErrorKind.unauthorized:
        return ErrorView(message: l10n.visualAidSignIn);

      case ApiErrorKind.forbidden:
        return _PromptView(
          icon: LucideIcons.arrowUpCircle,
          title: l10n.visualAidUpgradeTitle,
          body: l10n.visualAidUpgradeBody,
          actionLabel: l10n.visualAidSeePricing,
          onAction: openPricing,
        );

      case ApiErrorKind.rateLimited:
        // The daily cap is the image budget a teacher hits first, so it gets its
        ...
        return _PromptView(
          ...
          title: isDaily
              ? l10n.visualAidDailyLimitTitle
              : l10n.visualAidLimitTitle,
          body: isDaily
              ? l10n.visualAidDailyLimitBody
              : l10n.visualAidLimitBody,
          actionLabel: l10n.visualAidSeePricing,
          onAction: openPricing,
        );

      case ApiErrorKind.timeout:
        return ErrorView(message: l10n.visualAidTimeout, onRetry: onRetry);

      case ApiErrorKind.server:
        // 503 carries Retry-After; telling the teacher how long beats a bare
        // "try again". 504 (the slow-model image timeout) also lands here.
        final seconds = _retryAfterSeconds(api);
        return ErrorView(
          message: seconds == null
              ? l10n.visualAidBusy
              : l10n.visualAidBusyRetryAfter(seconds),
          onRetry: onRetry,
        );

      case ApiErrorKind.badResponse:
        // 422 IMAGE_GENERATION_EMPTY is its own recoverable state: the model ran
        // but produced no image, so the fix is fewer labels, not a clearer
        // photo. 400 is a safety violation or a rephrase.
        final message = switch (api.statusCode) {
          422 => l10n.visualAidEmptyGeneration,
          400 => l10n.visualAidRephrase,
          _ => l10n.errorGeneric,
        };
