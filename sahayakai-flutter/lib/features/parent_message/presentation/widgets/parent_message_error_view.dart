...
/// Maps a failed request to the *right* recovery UI, not a single generic error.
/// The network layer produces the typed [ApiException]; this widget decides
/// whether the teacher should retry, fill in a field, upgrade, wait or sign in.
///
...
///   - 400 Missing required fields-> a SPECIFIC "fill in student/class/subject/
///                                   reason/parent language" message + retry
///   - timeout / 5xx              -> retryable message + retry
class ParentMessageErrorView extends ConsumerWidget {
  const ParentMessageErrorView({super.key, required this.error, this.onRetry});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    switch (api.kind) {
      ...
      case ApiErrorKind.unauthorized:
        return ErrorView(message: l10n.parentMessageSignIn);

      case ApiErrorKind.forbidden:
        return _PromptView(
          icon: LucideIcons.arrowUpCircle,
          title: l10n.parentMessageUpgradeTitle,
          body: l10n.parentMessageUpgradeBody,
          actionLabel: l10n.parentMessageSeePricing,
      ...
      case ApiErrorKind.rateLimited:
        return _PromptView(
          ...
          title: l10n.parentMessageLimitTitle,
          body: l10n.parentMessageLimitBody,
          actionLabel: l10n.parentMessageSeePricing,
          onAction: openPricing,
        );

      case ApiErrorKind.timeout:
        return ErrorView(message: l10n.parentMessageTimeout, onRetry: onRetry);
      ...
      case ApiErrorKind.server:
        ...
        return ErrorView(
          message: seconds == null
              ? l10n.parentMessageBusy
              : l10n.parentMessageBusyRetryAfter(seconds),
          onRetry: onRetry,
        );

      case ApiErrorKind.badResponse:
        // The route returns 400 `{ error: 'Missing required fields' }` when a
        // required field is absent. The form validates these first, so this is
        // a defensive path — but it must read as a specific "fill these in",
        // never a generic failure (task requirement).
        final message = api.statusCode == 400
            ? l10n.parentMessageMissingFields
            : l10n.errorGeneric;
