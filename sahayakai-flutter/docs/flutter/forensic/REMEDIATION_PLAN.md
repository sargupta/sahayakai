...
**Status (2026-07-29) — option (a) DONE.** `VidyaAppBarAction` is now in the `GlassAppBar` of the five non-`ToolScaffold` surfaces that lacked it — Library, Pro Inbox, the conversation thread, Staffroom, and the Network hub (Profile already had it via `ToolScaffold`). So the voice launcher is reachable from every signed-in surface, the mobile analog of web's shell-mounted orb. Tests: `test/features/voice/launcher_ubiquity_test.dart` pins it on Inbox/Staffroom/Network hub; explicit assertions were added to the Library and conversation-thread suites (Library's items provider fetches, so it's asserted inside its own setup). `flutter analyze` 0, token_guard PASS, full suite only the 13 known create_palette failures. Option (b) — the single persistent floating overlay in `app_shell.dart` — remains the "right" version and is still open. Note: the Network hub app bar's actions slot was reserved for a future notifications bell; the launcher and that bell coexist in `actions` when the bell lands.
...
### P3.2 — Device-locale pre-selection — **DONE (2026-07-29)**; mother-tongue greeting is a FOUNDER decision (auto-play audio)
...
**Mother-tongue greeting — read the web source; it is a FOUNDER design decision on mobile, deliberately NOT auto-shipped (2026-07-29).** `mother-tongue-greeting.tsx` renders **nothing** — it is a behavior hook mounted globally in `app-shell.tsx:121`. On the first app open *after onboarding* it **auto-plays a TTS greeting with no tap** (`tts.speak(...)` 1.5s after render), once per user, gated on a persisted `profile.hasHeardGreeting` flag. Its whole point is the surprise: "This app speaks MY language."

Two reasons this is not a safe autonomous port:

1. **Unprompted audio is a real downside on a phone.** A teacher who opens the app in a staff room, a shared classroom, or on a bus would suddenly hear a voice from their phone with no gesture that asked for it — the exact "surprise audio" case to avoid. On the desktop web this is far lower-stakes than on a personal handset. Auto-play-without-a-gesture is also fragile against the mobile audio session and accessibility (reduce-motion) expectations. This is a genuine product call, not a mechanical port.
2. **The once-per-user gate has no mobile home.** The web flag lives on the server profile (`profile.hasHeardGreeting`); the mobile `TeacherProfile`/PATCH allowlist has **no such field**, so the "play only the first time" gate would have to persist locally (SharedPreferences) — an easy adaptation, but it confirms this isn't a drop-in.

**What's already delivered:** the *visible* half of the moment is present — `loginTitle`/`loginSubtitle` (and the onboarding titles) localise, now to the **device locale** (P3.2), so a Bengali teacher already reads a Bengali welcome on the first screen. The unshipped piece is strictly the auto-played *audio*.

**Recommendation for the founder (pick one; not auto-applied):** (a) **tap-to-hear** — render a small branded greeting line on the VIDYA home / first screen with a "listen" control reusing the P1 `ReadAloudButton`, so the audio plays only on an explicit tap (my recommendation — keeps the brand moment, no surprise audio, ~0.5 day, local `hasHeardGreeting` flag); (b) **faithful auto-play** — port the web behavior verbatim (autoplay 1.5s after first post-onboarding open, local once-flag), accepting the surprise-audio tradeoff on a handset (~0.5–1 day); (c) **skip** — the localized visible welcome is enough. The 11 greeting strings already exist in `mother-tongue-greeting.tsx:20-32` to reuse for (a)/(b).
