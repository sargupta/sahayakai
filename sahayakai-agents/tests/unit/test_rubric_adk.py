"""Unit tests pinning the ADK construct rubric builds (Phase U.alpha).

`build_rubric_agent()` returns a real `google.adk.agents.LlmAgent`.
Before Phase U.alpha the rubric router used a hand-rolled
...
Tests cover:
  ...
  - `output_schema=RubricGeneratorCore` so ADK switches Gemini into
...
def _build() -> object:
    ...
    for key in (
        "sahayakai_agents.agents.rubric.agent",
    ):
        sys.modules.pop(key, None)
    from sahayakai_agents.agents.rubric.agent import (  # noqa: PLC0415
        build_rubric_agent,
    )
    build_rubric_agent.cache_clear()
    return build_rubric_agent()


class TestBuildRubricAgent:
    def test_returns_llm_agent(self) -> None:
        ...
        assert isinstance(agent, LlmAgent), (
            "build_rubric_agent must return google.adk.agents.LlmAgent. "
    ...
    def test_caches_via_lru(self) -> None:
        ...
        second = build_rubric_agent()
        assert first is second, (
            "build_rubric_agent must cache its result so the agent is "
            "constructed once at import time, not per request."
        )

    def test_output_schema_is_rubric_generator_core(self) -> None:
        """Pinning the output_schema link is critical: ADK turns this
        into Gemini's `response_schema` so the model returns structured
        JSON matching `RubricGeneratorCore`."""
        from sahayakai_agents.agents.rubric.schemas import (  # noqa: PLC0415
            RubricGeneratorCore,
        )

        agent = _build()
        assert agent.output_schema is RubricGeneratorCore, (
            f"agent.output_schema must be RubricGeneratorCore. "
    ...
    def test_has_python_identifier_name(self) -> None:
        ...
        Pin snake_case so a regression to `rubric-generator` is caught
        at unit-test time."""
        agent = _build()
        ...
        assert "rubric" in agent.name.lower(), (
            f"Agent name should reference rubric. Got: {agent.name!r}"
        )
