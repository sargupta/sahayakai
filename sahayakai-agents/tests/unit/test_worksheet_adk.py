"""Unit tests pinning the ADK construct worksheet builds (Phase U.beta).

`build_worksheet_agent()` returns a real `google.adk.agents.LlmAgent`.
Before Phase U.beta the sidecar imported `google-adk` but did not USE
it for this router; the wizard stage was a hand-rolled
`google.genai.Client.aio.models.generate_content` call (multimodal:
image bytes + text prompt). These tests guard against regressing back
to that hand-rolled shape.

Tests cover:
  - The factory returns an `LlmAgent` instance.
  - The model string matches `get_wizard_model()`.
  - `lru_cache` semantics — calling twice returns the same instance.
  - `output_schema` is `WorksheetCore`.
...
def _build() -> object:
    for key in (
        "sahayakai_agents.agents.worksheet.agent",
    ):
        sys.modules.pop(key, None)
    from sahayakai_agents.agents.worksheet.agent import (  # noqa: PLC0415
        build_worksheet_agent,
    )
    return build_worksheet_agent()


class TestBuildWorksheetAgent:
    def test_returns_llm_agent(self) -> None:
        """Phase U.beta's whole point: the wizard is a real ADK
        `LlmAgent`, not a hand-rolled FastAPI handler."""
        from google.adk.agents import LlmAgent  # noqa: PLC0415

        agent = _build()
        assert isinstance(agent, LlmAgent), (
            "build_worksheet_agent must return google.adk.agents."
            f"LlmAgent. Got: {type(agent).__name__}"
        )

    def test_uses_wizard_model(self) -> None:
    ...
    def test_caches_via_lru(self) -> None:
        ...
        second = build_worksheet_agent()
        assert first is second, (
            "build_worksheet_agent must cache its result so the "
            "agent is constructed once at import time, not per request."
        )

    def test_output_schema_is_worksheet_core(self) -> None:
        """Pinning the output_schema link is critical: ADK turns this
        into Gemini's `response_schema` so the model returns structured
        JSON matching `WorksheetCore`. Drift here would silently return
        free-text the router can't parse."""
        from sahayakai_agents.agents.worksheet.schemas import (  # noqa: PLC0415
            WorksheetCore,
        )

        agent = _build()
        assert agent.output_schema is WorksheetCore, (
            f"agent.output_schema must be WorksheetCore. "
    ...
    def test_has_snake_case_name(self) -> None:
        ...
        assert "worksheet" in agent.name, (
